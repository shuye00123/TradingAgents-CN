...
def test_toolkit_binance_method():
    """测试工具包港股方法"""
    print("🧪 测试工具包港股方法...")
...
def test_binance_availability():
    """测试AKShare港股可用性"""
    print("\n🧪 测试binance可用性...")

    try:
        from tradingagents.dataflows.interface import BINANCE_AVAILABLE

        print(f"  binance可用: {BINANCE_AVAILABLE}")
        ...
        if BINANCE_AVAILABLE:
            ...
            print("  ⚠️ binance数据源不可用")
        try:
            print(f"🔄 优先使用binance api获取cypo数据: btcusdt")
            result = get_crypto_data("btcusdt",'1d','2025-08-11','2025-08-12')

            if result and "❌" not in result:
                print(f"✅ binance数据获取成功: {result}")
            else:
                print(f"⚠️ binance返回错误结果，尝试备用方案")
        except Exception as e:
            print(f"⚠️ binance数据获取失败: {e}")
        # 测试统一接口
        from tradingagents.dataflows.interface import get_cypo_stock_data_unified
        print("  ✅ binance统一接口导入成功")

        return True

    except Exception as e:
        print(f"❌ binance可用性测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False
